...
`layout.expand(component)` is shorthand for `layout.setFlexGrow(1, component)`. With multiple growing children, ratios distribute proportionally: `layout.setFlexGrow(2, a)` and `layout.setFlexGrow(1, b)` gives `a` twice the space of `b`.
