 ...
 *
 * --accept-reclassification allows a property that was read-only to become
 * customizable; without it, that change stops the run.
 *
...
async function generate(config) {
  ...
  const artifactPath = join(repositoryRoot, config.outputs.artifact);
  await verifyNoSilentDowngrade(properties, artifactPath, process.argv.includes('--accept-reclassification'));
...
/**
 * Refuses to turn a property that was read-only into a customizable one without
 * someone saying so.
 *
 * Reading badges out of prose can only ever recognize the markup it knows, so
 * the last line of defence cannot itself depend on recognizing markup. The
 * artifact already committed is a record of what every property was classified
 * as; a property crossing into "safe to write" is the one change that must
 * never happen quietly, whether the docs genuinely reclassified it or the
...
async function verifyNoSilentDowngrade(properties, artifactPath, accepted) {
  ...
  const downgraded = properties.filter((property) => property.writable && before.get(property.name)?.writable === false);
  if (downgraded.length === 0 || accepted) return;

  throw new Error(
    `${downgraded.length} property(ies) that were read-only are now reported as customizable:\n` +
      `  ${downgraded.map((property) => property.name).join('\n  ')}\n` +
      'Either the docs reclassified them, or their badge markup changed into a form lib/parse-docs.mjs cannot see ' +
      'and they are now wrongly marked safe to write. Check the docs, then re-run with --accept-reclassification.',
  );
}
